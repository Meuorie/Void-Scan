...
VOID SCAN – System Integrity Auditor

Author: Meuorie | Version: 2000.0 (Silent Engine)
...
self.version = "2000.0 (Silent Engine)"
...
[dim white] System Integrity Audit | Silent Engine | {self.author} [/dim white]

{ANIME_CAT}

"""

console.print(Panel(logo, border_style="white", box=box.SQUARE, padding=(1, 2)))
...
with Progress(SpinnerColumn(spinner_name="dots12"), TextColumn("[bold white]Analyzing System Integrity..."), BarColumn(bar_width=40), transient=True) as progress:
...
# السطر السحري: تجاهل الملفات الخاصة (sockets, pipes, devices)
...
console.print(f"\n[bold cyan]─── [ SYSTEM AUDIT REPORT ] ───[/bold cyan]\n")


threats = sum(len(v) for k, v in self.scan_results.items() if isinstance(v, list))

color = "green" if threats == 0 else "red"


score = max(0, 100 - (threats * 10))

console.print(Panel(f"[bold white]Integrity Score:[/bold white] [bold {color}]{score}%[/bold {color}] | [bold white]Anomalies:[/bold white] [bold {color}]{threats}[/bold {color}]", border_style="white", box=box.SQUARE))



if threats > 0:

t = Table(box=box.SIMPLE, header_style="bold white", border_style="white", expand=True)

t.add_column("Category", style="white", justify="center")
...
for p in self.scan_results["behavior"]: t.add_row("Behavior", str(p['PID']), p['Behavior'])

for f in self.scan_results["files"]: t.add_row("File", f['Path'], f['Reason'])

for m in self.scan_results["memory"]: t.add_row("Memory", "Segment", m)

for r in self.scan_results["rootkits"]: t.add_row("Kernel", "Core", r)
...
console.print(Panel("[bold green]Status: Clean – No threats identified.[/bold green]", border_style="green", box=box.SQUARE))
...
console.print(Panel("[bold yellow]Notice: Root privileges enable full system analysis.[/bold yellow]", border_style="yellow", box=box.MINIMAL))


with console.status("[bold white]Starting system audit..."):
...
console.print(f"\n[dim white]Audit completed at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | {self.author}[/dim white]")
